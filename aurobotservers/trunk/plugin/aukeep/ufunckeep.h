...
*/
class UFuncKeep : public UFuncPlugBase
...
public:
  ...
  { // command list and version text
    setCommand("keep keepWhenOn keepWhenOff", "keeper", "process maintenance (depricated) - use a rule instead");
    keep = NULL;
  }
  /**
  Destructor */
  ...
  bool handleKeep(UServerInMsg * msg);
  /**
   * Handle the keep push commands
   * \param msg pointer to the message and the client issuing the command
   * \param onCommand if true, the push relates to the on event, else the off event
   * \returns true if handled */
  bool handleKeepPush(UServerInMsg * msg, bool onCommand);
