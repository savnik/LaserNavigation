...
*/
class UFunctionSeq : public UFuncPlugBase
{ // NAMING convention recommend that the main plugin function class
  // starts with UFunction (as in UFunctionSeq) followed by
...
protected:
  ...
  bool seqLocal;
  /**
  Send a general status to client */
  bool sendStatusMessage(UServerInMsg * msg,
                         const char * tagName);
  /**
  Add a command line and report some errors.
  Returns true. */
  bool addCmdLine(UServerInMsg * msg, const char * line);

private:
  /**
  Function to get closest distance */
  bool handlePlanGet(UServerInMsg * msg);
  bool handlePlanSet(UServerInMsg * msg);
};
