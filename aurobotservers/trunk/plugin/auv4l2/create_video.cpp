...
static void alloc_videoBuff(GLOBAL *global)
{
  ...
  pthread_mutex_lock(&global->mutex);
    if (global->videoBuff == NULL)
    {
      ...
      global->videoBuff = (VidBuff *) malloc(sizeof(VidBuff) * global->video_buff_size);
      for(i=0;i<global->video_buff_size;i++)
      {
        global->videoBuff[i].frame = (BYTE*) malloc(sizeof(BYTE) * framesize);
      }
    }
    else
    {
      ...
      for(i = 0; i < global->video_buff_size; i++)
      {
        if(global->videoBuff[i].frame)
          free(global->videoBuff[i].frame);
        global->videoBuff[i].frame = NULL;
      }
      free(global->videoBuff);
...
static int initVideoFile(GLOBAL *global, vdIn *videoIn, void * lavc_aud_data)
{
...
/*	struct GLOBAL *global = all_data->global;
	...
	switch (global->VidFormat)
	{
		case AVI_FORMAT:
			...
			videoF->AviOut = (avi_t *) malloc(sizeof(struct avi_t));
			
			if(AVI_open_output_file(videoF->AviOut, videoIn->VidFName)<0) 
			{
				fprintf(stderr, "Error: Couldn't create Video.\n");
...
/*				if(global->Sound_enable > 0) 
				{
					...
					if(init_sound (pdata)) 
					{
						fprintf(stderr, "error opening portaudio\n");
...
//				set_sound(global, pdata, (void *) lavc_audio_data);
			...
			if(init_FormatContext(global, videoIn, videoF)<0)
			{
				capVid = false;
				pthread_mutex_lock(&videoIn->mutex);
					videoIn->capVid = capVid;
...
static void aviClose (GLOBAL *global, VideoFormatData *videoF)
{
	float tottime = 0;
...
// 	struct paRecordData *pdata = all_data->pdata;
	...
	if (!(videoF->AviOut->closed))
	{
		...
		if (global->debug) printf("stop= %lu start=%lu \n",
			(long unsigned int)global->Vidstoptime, (long unsigned int) global->Vidstarttime);
		if (tottime > 0) 
		{
		...
		if (global->debug) printf("VIDEO: %d frames in %f ms = %f fps\n",global->framecount,tottime,videoF->AviOut->fps);
		/*------------------- close audio stream and clean up -------------------*/
		if (global->Sound_enable > 0) 
		{
			//if (close_sound (pdata)) fprintf(stderr, "Sound Close error\n");
		} 
...
static void closeVideoFile(GLOBAL *global, vdIn *videoIn, VideoFormatData * videoF)
{
	//struct GLOBAL *global = all_data->global;
	//struct vdIn *videoIn = all_data->videoIn;
	//struct paRecordData *pdata = all_data->pdata;
...
/*	pthread_mutex_lock(pdata->mutex);
	...
	{
		case AVI_FORMAT:
			aviClose(global, videoF);
			break;
			
		case MKV_FORMAT:
			if(clean_FormatContext (global, videoF))
				fprintf(stderr, "matroska close returned a error\n");
			break;
...
static int write_video_frame (GLOBAL *global, 
                              void *jpeg_struct,
                              void *lavc_data,
                              VidBuff *proc_buff)
{
	...
	switch (global->VidFormat)
	{
		case AVI_FORMAT:
			...
			if(!(global->VidButtPress)) //if this is set AVI reached it's limit size
				ret = compress_frame(global, jpeg_struct, lavc_data, proc_buff);
...
/*					if (!(global->VidButtPress))
                    ...
                    printf("AVI file size limit reached\n"); // - restarted capture on new file\n");
//					}
				} 
				else 
				{
					fprintf(stdout, "write error on avi out \n");
				}
			}
			break;
		...
		case MKV_FORMAT:
			//global->framecount++;
			hertil
			ret = compress_frame(global, videoF,  jpeg_struct, lavc_data, proc_buff);
			break;
...
static int write_audio_frame (struct ALL_DATA *all_data, void *lavc_adata, AudBuff *proc_buff)
{
	...
	switch (global->VidFormat)
	{
		case AVI_FORMAT:
			...
			break;
		case MKV_FORMAT:
			pthread_mutex_lock( pdata->mutex ); //why do we need this ???
				/*set pts*/
			...
			{ 
				...
				{ 	/*only sync between 10ms and 2 seconds*/
					...
					{	/*shift sound by synctime*/
						...
						if (global->debug) printf("shift sound forward by %d samples\n",
							shiftSamples);
						short *EmptySamp;
			...
			struct audio_effects **aud_eff)
{
	...
	if(used)
	{
		...
		pthread_mutex_unlock( pdata->mutex ); /*now we should be able to unlock the audio mutex*/
				...
				struct JPEG_ENCODER_STRUCTURE **jpeg_struct)
{
	...
	if (used)
	{
		...
		{
			/* audio delayed */
			printf("audio drift: dropping/shifting frame\n");
			pthread_mutex_lock(global->mutex);
		...
		{
			/* audio too fast */
			printf("audio drift: duplicating/shifting frame\n");
			pthread_mutex_lock(global->mutex);
				global->av_drift -= max_drift;
			pthread_mutex_unlock(global->mutex);
...
int store_video_frame(void *data)
{
	...
	if(!global->videoBuff)
	{
		fprintf(stderr, "WARNING: video ring buffer not allocated yet - dropping frame.");
		pthread_mutex_unlock(global->mutex);
		return(-1);
	}
	...
	{
		if(global->debug) fprintf(stderr, "WARNING: buffer full waiting for free space\n");
...
void *IO_loop(void * )
{
    ...
    //static int initVideoFile(GLOBAL *global, vdIn *videoIn, VideoFormatData *videoF, void * lavc_aud_data)

	if(initVideoFile(global, videoIn, vid, (void *) &(lavc_audio_data))<0)
	{
		fprintf(stderr, "Cap Video failed\n");
		if(!(global->no_display))
        {
		    gdk_threads_enter();
	...
	{
		if(global->debug) printf("IO thread started...OK\n");
	...
	if(!failed)
	{
		while(!finished)
		{
			...
			if(finished)
			{
				...
				if( !(stall > 0) )
				{
					fprintf(stderr, "audio streaming stalled - timeout\n");
				}
		...
		/*finish capture*/
		closeVideoFile(global, videoIn, NULL);
	...
	if(global->debug) printf("IO thread finished...OK\n");
