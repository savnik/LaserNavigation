...
check_image_type (char *filename) 
{
  ...
  p1 = strrchr(filename,'.');
  /* change image type */
  if (strcasecmp(p1, ".jpg") == 0)
    format = 0;
  else if (strcasecmp(p1, ".jpeg") == 0)
    format = 0;
  else if (strcasecmp(p1, ".bmp") == 0)
    format = 1;
  else if (strcasecmp(p1, ".png") == 0)
    format = 2;
  else if (strcasecmp(p1, ".raw") == 0)
    format = 3;
  else
    format = 0;
...
char *setImgExt(char *filename, int format, int filenameCnt)
{
  int n;
  char * p1, *p2;
  p1 = strrchr(filename, '/');
