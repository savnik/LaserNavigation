...
int get_PixFormat(char *mode)
{
  ...
  for (i=0; i<SUP_PIX_FMT; i++)
  {
    ...
    if (strcmp(mode, listSupFormats[i].mode) == 0)
    {
        return (listSupFormats[i].format);
    }
  }
  return (listSupFormats[SUP_PIX_FMT - 2].format); /*default is- RGB8*/
}
...
LFormats *enum_frame_formats(int *width, int *height, int fd)
{
	...
	while ((ret = xioctl(fd, VIDIOC_ENUM_FMT, &fmt)) == 0) 
	{
		...
		{
			fprintf(stderr,"   { not supported - request format(%i) support at http://guvcview.berlios.de }\n",
						fmt.pixelformat);
		}
	}
	if (errno != EINVAL) {
