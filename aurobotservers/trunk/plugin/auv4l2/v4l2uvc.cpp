...
static int check_videoIn(struct vdIn *vd, int *width, int *height)
{
	...
	if(!(vd->listFormats->listVidFormats))
		fprintf(stderr, "Couldn't detect any supported formats on your device (%i)\n", vd->listFormats->numb_formats);
	return VDIN_OK;
}
...
int get_jpegcomp(struct vdIn *vd)
{
	...
	else
	{
		...
		if(errno == EINVAL)
		{
			...
			printf("   compression control not supported\n");
		}
	}
...
static int init_v4l2(struct vdIn *vd, int *format, int *width, int *height, int *fps, int *fps_num)
{
	...
	switch (vd->cap_meth)
	{
		case IO_READ: //allocate buffer for read
			...
			vd->mem[vd->buf.index] = malloc(sizeof(BYTE) * vd->buf.length);
			break;
...
static int videoIn_frame_alloca(struct vdIn *vd, int format, int width, int height)
{
  ...
  switch (format)
  {
      ...
      case V4L2_PIX_FMT_MJPEG:
        ...
        vd->tmpbuffer = (unsigned char*) malloc(sizeof(unsigned char) * tmpbuf_size);

        framebuf_size = width * (height + 8) * 2;
        vd->framebuffer = (unsigned char*)malloc(sizeof(unsigned char) * framebuf_size);
        break;
...
/*          vd->tmpbuffer = g_new0(unsigned char, tmpbuf_size);
  ...
  if ((!vd->framebuffer) || (framebuf_size <=0))
      {
          fprintf(stderr, "couldn't calloc %lu bytes of memory for frame buffer\n",
...
{
	int ret = VDIN_OK;
	const char *device = global->videodevice;
	...
	if(global->debug) printf("capture method = %i\n",vd->cap_meth);
	strncpy(vd->videodevice ,device, MAX_DEV_NAME_CNT);
	...
	if (vd->listDevices != NULL)
	{
		if(!(vd->listDevices->listVidDevices))
			fprintf(stderr, "unable to detect video devices on your system (%i)\n", vd->listDevices->num_devices);
	}
	else
		fprintf(stderr, "Unable to detect devices on your system\n");
	...
	if(vd->listDevices->num_devices > 0)
	{
		...
		if(strcmp(vd->listDevices->listVidDevices[vd->listDevices->current_device].driver,"uvcvideo") == 0)
		{
...
static int frame_decode(struct vdIn *vd, int format, int width, int height, 
                        unsigned char * jpegBuffer = NULL, int * jpegBufferCnt = 0)
{
	int ret = VDIN_OK;
	int framesizeIn =(width * height << 1);//2 bytes per pixel
        unsigned char * jbuf;
	...
	switch (format) 
	{
		...
		case V4L2_PIX_FMT_MJPEG:
			...
			if ((uint32_t)*jpegBufferCnt >= vd->buf.bytesused)
                        {
                          jbuf = jpegBuffer;
                          jpegSize = vd->buf.bytesused;
                        }
                        else
                          jbuf = vd->tmpbuffer;
			memcpy(jbuf, vd->mem[vd->buf.index],vd->buf.bytesused);

			if (jpeg_decode(&vd->framebuffer, jbuf, width, height) < 0) 
			{
		...
		case V4L2_PIX_FMT_YUYV:
			if(vd->isbayer>0) 
			{
				...
				{
					// rgb buffer for decoding bayer data
					vd->tmpbuffer = (unsigned char*) malloc(sizeof(unsigned char) *
						width * height * 3);
				}
				bayer_to_rgb24 ((BYTE*)vd->mem[vd->buf.index],vd->tmpbuffer, width, height, vd->pix_order);
				// raw bayer is only available in logitech cameras in yuyv mode
				rgb2yuyv (vd->tmpbuffer,vd->framebuffer, width, height);
			} 
			else 
			{
				if ((int)vd->buf.bytesused > framesizeIn)
		...
		case V4L2_PIX_FMT_SGBRG8: //0
			bayer_to_rgb24 ((BYTE*)vd->mem[vd->buf.index],vd->tmpbuffer, width, height, 0);
			rgb2yuyv (vd->tmpbuffer, vd->framebuffer, width, height);
			break;
			
		case V4L2_PIX_FMT_SGRBG8: //1
			bayer_to_rgb24 ((BYTE*)vd->mem[vd->buf.index], vd->tmpbuffer, width, height, 1);
			rgb2yuyv (vd->tmpbuffer, vd->framebuffer, width, height);
			break;
			
		case V4L2_PIX_FMT_SBGGR8: //2
			bayer_to_rgb24 ((BYTE*)vd->mem[vd->buf.index], vd->tmpbuffer, width, height, 2);
			rgb2yuyv (vd->tmpbuffer, vd->framebuffer, width, height);
			break;
		case V4L2_PIX_FMT_SRGGB8: //3
			bayer_to_rgb24 ((BYTE*)vd->mem[vd->buf.index], vd->tmpbuffer, width, height, 3);
			rgb2yuyv (vd->tmpbuffer, vd->framebuffer, width, height);
			break;
		...
		default:
			fprintf(stderr, "error grabbing (v4l2uvc.c) unknown format: %i\n", format);
			ret = VDIN_UNKNOWN_ERR;
			goto err;
			break;
	}
	*jpegBufferCnt = jpegSize;
err:
	return ret;
}
...
int uvcGrab(struct vdIn *vd, int format, int width, int height, int *fps, int *fps_num,
                unsigned char * jbuff, int * jbuffCnt)
{
  int ret = VDIN_OK;
  fd_set rdset;
  ...
  else if ((ret > 0) && (FD_ISSET(vd->fd, &rdset)))
  {
    switch(vd->cap_meth)
    {
      case IO_READ:
        ...
        gettimeofday(&vd->timestamp, NULL); // ns_time_monotonic();
        if (-1 == (int)vd->buf.bytesused )
        {
          switch (errno)
          {
            case EAGAIN:
...
/* if (vd->cap_raw > 0)
  ...
  if ((ret = frame_decode(vd, format, width, height, jbuff, jbuffCnt)) != VDIN_OK)
  {
      goto err;
  }
...
{
	//clean frame buffers
	if(vd->tmpbuffer != NULL) free(vd->tmpbuffer);
	vd->tmpbuffer = NULL;
	if(vd->framebuffer != NULL) free(vd->framebuffer);
	vd->framebuffer = NULL;
	// unmap queue buffers
	switch(vd->cap_meth)
	{
		...
		default:
			...
			if(xioctl(vd->fd, VIDIOC_REQBUFS, &vd->rb)<0)
			{
				fprintf(stderr, "VIDIOC_REQBUFS - Failed to delete buffers: %s (errno %d)\n", strerror(errno), errno);
				return(VDIN_REQBUFS_ERR);	
			}
			break;
	}
...
{  
	...
	if (ret < 0) 
	{
		fprintf(stderr, "Unable to set %d/%d fps\n", *fps_num, *fps);
