...
LDevices *list_devices( gchar *videodevice )
{
  ...
  listDevices = (LDevices *) malloc(sizeof(LDevices));
  ...
  const int MDL = 500;
  char v4l2_device[MDL];
  int num_dev = 0;
  int vidDev = 0;
  snprintf(v4l2_device, MDL, "video%d", vidDev);
  while(num_dev < MAX_DEV_CNT)
  {
    char device[MDL];
    snprintf(device, MDL, "/dev/%s", v4l2_device);
    //device = g_strjoin("/","/dev",v4l2_device,NULL);

    if ((fd = v4l2_open(device, O_RDWR | O_NONBLOCK, 0)) < 0)
    {
        fprintf(stderr, "ERROR opening V4L interface for %s\n",
            device);
        break;
//			continue; /*next dir entry*/
    ...
    else
    {
      ...
      {
        strncpy(listDevices->listVidDevices[num_dev].device, device, MAX_DEV_LEN);
        strncpy(listDevices->listVidDevices[num_dev].name, (char *) v4l2_cap.card, MAX_NAM_LEN);
        strncpy(listDevices->listVidDevices[num_dev].driver, (char *)v4l2_cap.driver, MAX_DRV_LEN);
        strncpy(listDevices->listVidDevices[num_dev].location, (char *)v4l2_cap.bus_info, MAX_LOC_LEN);
        listDevices->listVidDevices[num_dev].valid = 1;
        if(strcpy(videodevice, listDevices->listVidDevices[num_dev].device) == 0)
        {
          listDevices->listVidDevices[num_dev].current = 1;
          listDevices->current_device = num_dev;
        }
        else
          listDevices->listVidDevices[num_dev].current = 0;
        num_dev++;
        printf("%s - device %d\n", device, num_dev);
      }
    }
...
LDevices *enum_devices( gchar *videodevice, struct udev *udev, int debug)
{
    ...
    if (!udev) 
    {
        ...
        printf("Can't create udev...using sysfs method\n");
    ...
    udev_list_entry_foreach(dev_list_entry, devices) 
    {
        ...
        {
            fprintf(stderr, "ERROR opening V4L2 interface for %s\n", v4l2_device);
            v4l2_close(fd);
            continue; /*next dir entry*/
        }
        ...
        {
            perror("VIDIOC_QUERYCAP error");
            fprintf(stderr, "   couldn't query device %s\n", v4l2_device);
            v4l2_close(fd);
            continue; /*next dir entry*/
        }
        v4l2_close(fd);
        ...
        strncpy(listDevices->listVidDevices[num_dev].device, v4l2_device, MAX_DEV_LEN);
        strncpy(listDevices->listVidDevices[num_dev].name, (char *) v4l2_cap.card, MAX_NAM_LEN);
        strncpy(listDevices->listVidDevices[num_dev].driver, (char *)v4l2_cap.driver, MAX_DRV_LEN);
        strncpy(listDevices->listVidDevices[num_dev].location, (char *)v4l2_cap.bus_info, MAX_LOC_LEN);
/*        listDevices->listVidDevices[num_dev-1].device = g_strdup(v4l2_device);
        ...
        listDevices->listVidDevices[num_dev].valid = 1;
        if(strcmp(videodevice,listDevices->listVidDevices[num_dev].device) == 0)
        {
            listDevices->listVidDevices[num_dev].current = 1;
            listDevices->current_device = num_dev;
        }
            ...
            listDevices->listVidDevices[num_dev].current = 0;
        if (num_dev < MAX_DEV_CNT)
          num_dev++;
        else
          fprintf(stderr, "Too many (>%d) devices for fixed size structure!\n", MAX_DEV_CNT);
        ...
        const char * vs = udev_device_get_sysattr_value(dev,"idVendor");
        const char * ps = udev_device_get_sysattr_value(dev, "idProduct");
        listDevices->listVidDevices[num_dev-1].vendor = strtol(vs, NULL, 16);
        listDevices->listVidDevices[num_dev-1].product = strtol(ps, NULL, 16);
        if (debug)
          printf("Vendor %s, product %s, (%d %d)\n", vs, ps,
                 listDevices->listVidDevices[num_dev-1].vendor,
                 listDevices->listVidDevices[num_dev-1].product);
