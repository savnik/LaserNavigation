...
struct GLOBAL 
{
	pthread_mutex_t mutex;         //global structure mutex
//	GCond   *IO_cond;      //IO thread semaphore
	
	VidBuff *videoBuff;    //video Buffer
	int video_buff_size;   //size in frmaes of video buffer
	
	const char *videodevice;     // video device (def. /dev/video0)
	char *confPath;        //configuration file path
	char *vidfile;         //video filename passed through argument options with -n
	//char *WVcaption[32];       //video preview title bar caption
	//char *imageinc_str;    //label for File inc
	//char *vidinc_str;      //label for File inc
	char mode[6];            //mjpg (default)
	pchar vidFPath[2];       //video path [0] - filename  [1] - dir
	pchar imgFPath[2];       //image path [0] - filename  [1] - dir
	pchar profile_FPath[2];  //profile path [0] - filename  [1] - dir
