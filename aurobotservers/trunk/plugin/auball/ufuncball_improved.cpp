...
*/
class UFuncBall_improved : public UFunctionCamBase
...
public:
  ...
  UFuncBall_improved()
  {
    setCommand("ball_improved", "ball_improved", "camera based ball detect (compiled " __DATE__ " " __TIME__ ")");
  ...
  { // handle command(s) send to this plug-in
    ...
    { // create the reply in XML-like (html - like) format
      sendHelpStart("B2_improved");
      sendText("--- available B2_improved options\n");
...
Most of the code used is from the original code example.
  \param srcImg is a pointer to the image to be analyzed
  \param dstImg is where the pixels with the ball color is saved.
  \param debug if true, then more images are produced in the image pool
  ...
  void maskColors(UImage * srcImg, UImage * dstImg, bool blue)
  {
    ...
    // get threshold values (in 0..255 range) from global var settings
    if (blue)
      var = varBlueLim;
    else
      var = varRedLim;
    ...
    // apply given colour threshold and put result into image 2
    for (row = 0; row < (int)srcImg->getHeight(); row++) //for each row in picture
    { // get pixel pointer (3-plane) to source image start of row
      pixRGB = srcImg->getLine(row);
      // get pointer to destination pixel (same row)
      pixMask = (unsigned char *) dstImg->getLine(row);
      for (col = 0; col < (int)srcImg->getWidth(); col++) //for each pixel in the row
      { // plane 1 (p1) is R, plane 2 (p2) is G and plane 3 (p3) is B
	if(!blue)
	{
		if (row > topLine and
		pixRGB->p1 >= rMin and                // pixel red value >= minimum value
		pixRGB->p1 <= rMax and                // pixel red value <= max value
		pixRGB->p1 > ((pixRGB->p2)+((pixRGB->p2)-(pixRGB->p3))) // distance red-green > distance green-blue
		)

		*pixMask = 255; //set pixel as white (positive color match)
		else
		*pixMask = 0; //set negative match (black)
...
*/
  ...
  //  varRedBallDarker = addVar("redDarker", 1.6, "d", "(r/w) ball is to be this value darker than average Y value [0..1]");
  //  varBlueBallDarker = addVar("blueDarker", 1.1, "d", "(r/w) ball is to be this value darker than average Y value [0..1]");
...
{ // called by server to create an object of this type
  /** replace 'UFuncBall_improved' with your classname, as used in the headerfile */
  return new UFuncBall_improved();
}
#endif
