...
if ( cPlanes != 1 )
	return GBM_ERR_BMP_PLANES;
// added 32 bit test
if ( cBitCount != 1 && cBitCount != 4 && cBitCount != 8 && cBitCount != 24 && cBitCount != 32)
	return GBM_ERR_BMP_BITCOUNT;
    ...
    case BCA_RLE4:
      {
    	...
    	break;
    default:
      printf("readBMP unsupported format: ulCompression = %d\n", (int) bmp_priv->ulCompression);
...
GBM_ERR bmp_w(const char *fn, int fd, const GBM *gbm,
      const GBMRGB * /*gbmrgb*/, const byte *data, const char * /*opt*/)
{
// can only write 24 bit per pixel windows 2.0 bitaps
// (opt = "")
