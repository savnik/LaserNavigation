...
//   char * cn = NULL;
//   unsigned char * imdata1;
//   unsigned char * imdata2;
//   //unsigned int r,c;
//   //bool isOK = false;
//   //const char path[] = "/home/jca/jca/images/";
//   int i;
...
//     if (result)
//       printf("Camera open in (320 x 240) .... waiting 3 secs to settle\n");
//     else
...
//       imdata1 = raw.getData();
//       imdata2 = img.getData();
//       memcpy(imdata2, imdata1, raw.getDataSize());
//       */
//       img.copy(&raw);
//     }
//     t2.Now();
//     printf("Mem copy of 320x240 image takes %f ms on a 2.4 GHz PC\n", (t2 - t1)/10.0);
//   }
//   //
//   return result;
// }
...
//   UTime t1, t2;
//   //
//   printf("Testing getting image to 160x120 (800x600) buffer\n");
//   printf("Setting device number ...\n");
//   //
//   cam.setDeviceNumber(0);
//   printf("Getting camera name ... \n");
...
//   if (result)
//   {
//     result = cam.openAndSetDevice(160, 120, 5);
//     if (result)
//       printf("Camera open in (%d x %d) .... waiting 3 secs to settle\n",
...
//   if (result)
//   {
//     printf("Move image to UImage (RGB) type buffer\n");
//     raw.moveToRGB(&img, false);
//     result = img.saveBMP(imagePath, "testimage",160,"120");
//     if (result)
//       printf("Saved to '%s/testimage-%d-120.bmp'\n", imagePath, 160);
//     else
//       printf("Failed to save to '%s/testimage-%d-120.bmp'\n", imagePath, 160);
//     printf("Resize image to mas size\n");
//     img8.copyToMaxRes(&img);
//     result = img8.saveBMP(imagePath, "testimage",800,"600");
//     if (result)
//       printf("Saved to '%s/testimage-%d-600.bmp'\n", imagePath, 800);
//     else
//       printf("Failed to save to '%s/testimage-%d-600.bmp'\n", imagePath, 800);
//   }
//   //
//   return result;
// }
...
void testPlane()
{
  ...
  pl.set(p1, p2, p3);
  printf("%s\n", pl.print("", s, MSL));
  // set plane from 3 positions
  p1.set(0.0, 1.0, -0.5);
  p2.set(0.0, -1.0, -0.5);
  p3.set(1.0, 0.0, -0.5);
  pl.set(p1, p2, p3);
  printf("%s\n", pl.print("", s, MSL));
  // set plane from position and norm vector
  p1.set(0.0, 1.0, -0.5);
  n1.set(0.0, 0.0, 0.5);
  pl.set(p1, n1);
  printf("%s\n", pl.print("", s, MSL));
  // set plane from position of closest point
  p1.set(0.0, 0.0, -0.5);
  pl.set(p1);
  printf("%s\n", pl.print("", s, MSL));
  // distance to plane
  p1.set(1.0, 1.0, 2.0);
  printf("Dist to plane at -0.5= %g (should be 2.5)\n", pl.distSigned(p1));
  // distance to plane
  p1.set(100.0, 10.0, -2.0);
  printf("Dist to plane at -0.5= %g (should be 1.5)\n", pl.distSigned(p1));
  // set plane from position of closest point
  p1.set(0.0, 0.0, 0.5);
  pl.set(p1);
  printf("%s\n", pl.print("", s, MSL));
  ...
  p1.set(100.0, 10.0, -2.0);
  printf("Dist to plane at 0.5= %g (should be 2.5)\n", pl.distSigned(p1));
  // position off a plane
  pl.set(0.0, 0.0, -0.5);
  p2.set(3.0, 4.0, 7.0);
  p3 = pl.getOnPlane(p2);
  printf("Point on plane is (%g,%g,%g) should be (3,4,-0.5)\n", p3.x, p3.y, p3.z);
  p2.set(3.0, 4.0, -7.0);
  p3 = pl.getOnPlane(p2);
  printf("Point on plane is (%g,%g,%g) should be (3,4,-0.5)\n", p3.x, p3.y, p3.z);
  //
  // crossing of 2 planes
  pl.set(0.0, 0.0, 0.5);
  pm.set(0.0, 0.5, 0.0);
  line = pl.getPlaneCrossing(pm, &isOK);
  printf("Plane crossings is line: (%g,%g,%g) vec:(%g,%g,%g) ok=%d - should be pos(0,0.5,0.5) vec(+/-1.0,0.0)\n",
         line.pos.x, line.pos.y, line.pos.z, line.vec.x, line.vec.y, line.vec.z, isOK);
  // crossing of 2 planes
  pl.set(-0.5, 0.0, 0.0);
  pm.set( 0.0, 0.5, 0.0);
  line = pl.getPlaneCrossing(pm, &isOK);
  printf("Plane crossings is line: (%g,%g,%g) vec:(%g,%g,%g) ok=%d - should be pos(-0.5,0.5,0.0) vec(0,0,+/-1.0)\n",
         line.pos.x, line.pos.y, line.pos.z, line.vec.x, line.vec.y, line.vec.z, isOK);
  // crossing of 2 parallel planes
  pl.set(0.0, -0.5, 0.0);
  pm.set(0.0, 0.5, 0.0);
  line = pl.getPlaneCrossing(pm, &isOK);
  printf("Plane crossings is line: (%g,%g,%g) vec:(%g,%g,%g) ok=%d - should not be ok (parallel planes)\n",
         line.pos.x, line.pos.y, line.pos.z, line.vec.x, line.vec.y, line.vec.z, isOK);
}
...
void testLLtoUTM()
{
  double lat = 55.669877;
  double lon = 12.305591;
  double northing, easting, lat2, lon2;
  double nor2, east2;
  int zone = 32;
  //
  latlon2UTM(lat, lon, zone, &easting, &northing);
  utm2latlon(easting, northing, zone, &lat2, &lon2);
  latlon2UTM(lat2, lon2, zone, &east2, &nor2);
  printf("Lat lat,long: %.8fN and %.8fE\n", lat, lon);
  printf("UTM Nor,East: %.2fN and %.2fE zone %d\n", northing, easting, zone);
  printf("back lat,lon: %.8fN and %.8fE\n", lat2, lon2);
  printf("back Nor,Est: %.2fN and %.2fE zone %d\n", nor2, east2, zone);
  printf("diff lat: %.10f long: %.10f east %.3fm north %.3fm\n", lat - lat2, lon - lon2, easting - east2, northing - nor2);
}
...
void testStringSep()
{
  const char * s1 = "  \"this is a,\" but ends before 'but'";
  const char * s2 = "  \"this is \\\" a 'nest \\\" \"nest2\t88\" numbers, tab, nl \n ' and \\\"the rest \" \n\t\t ' a bit more' uha";
  const int MSL = 200;
  char sd[MSL];
  const char * p1;
  bool overflow;
  //
  printf("source 1 >%s<\n", s1);
  p1 = stringGet(s1, sd, MSL, &overflow);
  printf("result: >%s<\n", sd);
  printf("returned >%s<\n", p1);
  printf("Overflow: %s\n", bool2str(overflow));
  printf("--------\n");
  printf("source 2 >%s<\n", s2);
  p1 = stringGet(s2, sd, MSL, &overflow);
  printf("result: >%s<\n", sd);
  printf("returned >%s<\n", p1);
  printf("Overflow: %s\n", bool2str(overflow));
  printf("--------\n");
...
void test2Dseg()
{
  ...
  int n;
  //
  sb.setFromPoints(2.5, 1.6, 3.0, 2.0);
  d = sb.distanceSigned(2.6, 1.6);
  printf("Distance signed from bare line is %.4f, should be negative\n", d);
  //
  pa.set(3.0, 1.0);
  pb.set(0.0, 2.0);
  sa.setFromPoints(pa, pb);
  a = sa.getPositionOnLine(pc);
  printf("%s\n", sa.print("2Dline segment", s, MSL));
  sa.setFromPose(10.5, 0.0, -1.2);
  n = sa.getCircleCrossings(0.0, 4.0, 8.0, &t1, &t2);
  if (n == 0)
    printf("line %s is not crossing circle\n", sa.print("line: ", s, MSL));
  else
  { // get positions in x,y
    pa = sa.getPositionOnLine(t1);
    pb = sa.getPositionOnLine(t2);
    printf("line %s is crossing at %g and %g\n", sa.print("line: ", s, MSL), t1, t2);
    printf("Crossing 1: %gx,%gy)\n", pa.x, pa.y);
    printf("Crossing 2: %gx,%gy)\n", pb.x, pb.y);
  }
}
...
void testMatrixInv()
{
  UMatrix4 a(2,2), ai(2,2), b(2,2), bi(2,2);
  a.setRow(0, 1.0, 2.0);
  a.setRow(1, 3.0, 4.0);
  ai.inverse(&a);
  b = ai;
  bi.inverse(&b);
  printf("test of matrix inverse - first and last should be equal\n");
  a.print("a");
  ai.print("a inverted");
  b.print("b = a inverted");
  bi.print("b inverted - same as a");
  printf("ended\n");
  a.setSize(3,3);
  ai.setSize(3,3);
  bi.setSize(3,3);
  a.setRow(0, 2.231831, 0.201372, -1e-8);
  a.setRow(1, 0.201372, 0.018169, -1e-9);
  a.setRow(2, -1e-11,     -1e-10, 0.121846);
  ai.inverse(&a);
  b = ai;
  bi.inverse(&b);
  printf("test of matrix inverse - first and last should be equal\n");
