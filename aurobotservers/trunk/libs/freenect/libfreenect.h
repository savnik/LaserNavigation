...
/**
 * convert packed 11 bit depth to 16 bit */
FREENECTAPI void convert_packed11_to_16bit(uint8_t *raw, uint16_t *frame, int n);
FREENECTAPI void convert_packed_to_16bit(uint8_t *src, uint16_t *dest, int vw, int n);
FREENECTAPI void convert_packed_to_8bit(uint8_t *src, uint8_t *dest, int vw, int n);

// chr
struct deviceUseInfo {
  int useVideoFrame;
  int useVideoCnt;
  int useDepthFrame;
  int useDepthCnt;
  int syncVideoFromDepth;
  int videoIdle;
  int depthIdle;
  struct timeval videoFrameTime, depthFrameTime;
};
...
//         struct deviceUseInfoT * devUse;
// };
extern struct deviceUseInfo devUse[];
/**
 * Get the image usage and statistics structure from the context structure */
extern struct deviceUseInfo * getCtxUse(freenect_context *ctx);
/**
 * Get the image usage and statistics structure from the device structure */
extern struct deviceUseInfo * getDevUse(freenect_device *dev);

/**
 * max kinect devices in use simultaniously */
#define KINECT_MAX_DEVICE_USE 4
// chr end
