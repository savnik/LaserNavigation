...
// chr for device usage and status
struct deviceUseInfo devUse[KINECT_MAX_DEVICE_USE];
struct deviceUseInfo * getCtxUse(freenect_context *ctx)
{
  return ctx->devUse;
}
struct deviceUseInfo * getDevUse(freenect_device *dev)
{
  return dev->parent->devUse;
}
// chr end
...
static int stream_process(freenect_context *ctx, packet_stream *strm, uint8_t *pkt, int len, /*chr*/ int depth)
{
	if (len < 12)
		return 0;
    ...
    // 			return 0;
    // 		}
    // begin chr first if only
    if (hdr->flag != sof && ctx->devUse->useVideoFrame > 1)
      // not all frames are wanted, so just return dropping the packet
      return 0;
    else if (hdr->flag != sof) {
            FN_SPEW("[Stream %02x] Not synced yet...\n", strm->flag);
            return 0;
    }
    // begin chr
    // we have the first packet in a new frame
    if (depth)
    {
      ctx->devUse->useDepthCnt++;
      if (ctx->devUse->useDepthCnt < ctx->devUse->useDepthFrame)
        // stay in not synced mode
        return 0;
      else if (ctx->devUse->syncVideoFromDepth)
        // set video to use next available frame
        ctx->devUse->useVideoCnt = ctx->devUse->useVideoFrame;
      // use this depth frame and reset skip counter
      ctx->devUse->useDepthCnt = 0;
      // get frame time
      gettimeofday(&ctx->devUse->depthFrameTime, NULL);
      ctx->devUse->depthIdle = 0;
    }
    else
    {
      if (ctx->devUse->syncVideoFromDepth == 0 || ctx->devUse->useVideoFrame == 1)
        ctx->devUse->useVideoCnt++;
      if (ctx->devUse->useVideoCnt < ctx->devUse->useVideoFrame)
        // stay in not synced mode
        return 0;
      // use this frame and reset skip counter
      ctx->devUse->useVideoCnt = 0;
      // get frame time
      gettimeofday(&ctx->devUse->videoFrameTime, NULL);
      ctx->devUse->videoIdle = 0;
    }
    //  end chr
	  strm->synced = 1;
	  strm->seq = hdr->seq;
  ...
  // begin chr
  if (hdr->flag == eof)
  { // set idle flag to between frames
    if (depth)
      ctx->devUse->depthIdle = 1;
    else
      ctx->devUse->videoIdle = 1;
  }
  if (hdr->flag == eof && ((ctx->devUse->useVideoFrame > 1 && ! depth) || (ctx->devUse->useDepthFrame > 1 && depth)))
  { // this is the last packet in a used frame, then we do not want the next frame
    strm->synced = 0;
  }
  // end chr
...
static void depth_process(freenect_device *dev, uint8_t *pkt, int len)
{
	...
	int got_frame_size = stream_process(ctx, &dev->depth, pkt, len, 1);
...
static void video_process(freenect_device *dev, uint8_t *pkt, int len)
{
	...
	int got_frame_size = stream_process(ctx, &dev->video, pkt, len, 0);
