...
class svsAcquireImages
{
 public:
  ...
  IMPORT void SetThresh(int n);	// set texture filter threshold
  IMPORT void SetUnique(int n);	// set uniqueness filter threshold
  IMPORT void SetCorrsize(int n); // set correlation window size, (5) 7 to 21
...
: public svsAcquireImages
{
 public:
  ...
  virtual bool SetBalance(bool, int rd, int bl) = 0;
  virtual bool SetGamma(int gam) = 0;
  virtual bool SetBrightness(bool, int bright) = 0;
...
// opens a stream file, with a number of buffers, and optional user info
 ...
 protected:
  ...
  int logFD;
  char logName[256];
  int fsize; int fsize2; //long64 fsize; // __int64 fsize; // long long fsize;
  int bufsize;
  int blocksize;
  char *diskbuf;
