...
void smrsdl_drawpolylines(polylinestype p) {
  ...
  for (i = 0; i < p->numlines; i++) {
    for (j = 0; j < p->lines[i]->numpoints-1; j++) {
      switch(p->lines[i]->subtype) {
      case PLT_POLYLINE:
	r = 0;
	g = 255;
	b = 0;
	...
	break;
      ...
      DrawLine(screen,p->lines[i]->x[j] * xratio + width/2, p->lines[i]->y[j] * yratio + height/2,
	       p->lines[i]->x[j+1] * xratio + width/2, p->lines[i]->y[j+1] * yratio + height/2,
	       r,g,b);
    }
  }
}
