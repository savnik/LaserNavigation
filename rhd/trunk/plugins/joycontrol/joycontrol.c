 ...
 ***************************************************************************/
/************************** Library version  ***************************/
#define JOYCONTROLVERSION "1.1"
/************************** Version control information ***************************/
 #define REVISION         "$Rev: 249 $:"
...
int initJoyCtrl(void) {
  ...
  { // there should be something to control
    ...
    { // no control set of variables - exit
      printf("   ERROR: Looked for Ackerman, differential or Vel-omega control variables!\n");
...
extern int periodic(int tick)
{
  ...
  (joyValues.button[3])|| (joyValues.button[4])) {
    ...
    if ((joyValues.axes[2] > -32767) || (joyValues.axes[5] > -32767)) { //Left stick and going backwards
    ...
    } else if ((fabs(joyValues.axes[4]) > ctrlConfig.deadmax) || (fabs(joyValues.axes[3]) > ctrlConfig.deadmax)) { //Right stick control
