...
{  
  ...
  union 
  {
	...
	/*Send Keep-Auto-Mode, every cycle*/
	//msg.id=KEEPAUTOMODE;
	//msg.id=0x070;
	//msg.flags=0;
	//msg.cob=0;
	...
	//msg.data[1]=0x0;  // Maybe adding this zero in the end, helps with the problem
	//ret=secureWrite(can_dev,&msg, sizeof(struct canmsg_t));
	/* Try with number or another use of sizeof and the struct. */
	//ret=secureWrite(can_dev,&msg, 32);
	//if (ret<0) 
	//{
	...
	//Request quadrature encoder reading
	msg.id=0x110;
	msg.flags=0;
	msg.length=0;
	msg.data[0]=0x0;
