...
/** \fn void ludcmp(matrix *a, int N, int *indx, double *d)
...
void ludcmp(matrix *a, int N, int *indx, double *d)
{
	...
	*d = 1.0;
	for (i=0; i<=(N-1); i++) {
		...
		if (big==0.0) merror("Singular matrix i routine LUDCMP");
			vv->mat[i][0]=1.0/big;
	}
	for (j=0; j<=(N-1); j++) {
		for (i=0; i<j; i++) {
...
/** \fn void lubksb( matrix *a, int N, int *indx, double b[] )
 * LU-backsubstituting.
 * This function is used by MINV.
 *
...
/** \def SWAP(a,b)
 * Swaps the double values a and b using a temp value.
...
void gaussj( matrix *a, int n, matrix *b, int m )
{
	...
	for (i=0;i<n;i++) {
		...
		for (j=0;j<n;j++)
			if (ipiv->mat[0][j] != 1)
				for (k=0;k<n;k++) {
					...
					} else if (ipiv->mat[0][k] > 1) merror("GAUSSJ: Singular Matrix-1");
				}
		++(ipiv->mat[0][icol]);
		if (irow != icol) {
		...
		indxr->mat[0][i]=irow;
		indxc->mat[0][i]=icol;
		if (a->mat[icol][icol] == 0.0) merror("GAUSSJ: Singular Matrix-2");
