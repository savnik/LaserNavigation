...
typedef struct {
	...
	double **mat; ///< Array of pointers to the cloumns in the matrix (data storage).
} matrix;
...
void array2mat( matrix *ptv, double *values, int n );
...
double slload( varlisttype *varlist, char scal_name[] );
#if IO == ON
 #include <stdio.h>
...
void minvgauss( matrix *ptm1, matrix *ptm2 );
