...
void minit( matrix *ptm )
{
        ...
        bytes = sizeof(double)*(ptm->row)*(ptm->col);

        /* Set data area to all zeroes. */
        memset( ptm->mat[0], zero_byte, bytes );
}

/** \fn void minitx( matrix *ptm, double x )
...
/** \fn void mdiag( matrix *ptm, double diag )
...
void mset( matrix *ptm1, const matrix *ptm2 )
{
        ...
        bytes = sizeof(double)*(ptm1->row)*(ptm1->col);
...
/** \fn void smul( matrix *ptm1, matrix *ptm2, double factor)
 ...
 */
void smul( matrix *ptm1, const matrix *ptm2, double factor)
{
        register int i,j;
...
/** \fn void osmul( matrix *ptm1, matrix *ptm2, double factor)
 * Multiply matrix with scalar ptm1 = factor*ptm2 \n
 * Pointer based version, which should improve speed.
 *
 * \param[out] *ptm1 Pointer to result matrix.
 ...
 * \todo Test the function for functionality and speed. 
 */
void osmul( matrix *ptm1, matrix *ptm2, double factor)
{
...
/** \fn double sprod( matrix* ptv1, matrix* ptv2 )
 ...
 */
double sprod( const matrix* ptv1, const matrix* ptv2 )
{
...
/** \fn double osprod( matrix* ptv1, matrix* ptv2 )
 * This function calculates the scalar-product of two vectors of equal
 * length. The vectors may be either row- or column- vectors or any 
 * combination of those. (The usual mmul routine can be used to calculate
 * scalar-products, but it requires a row- and a column-vector.) \n
 ...
 * \todo Test the function for functionality and speed. 
 */
double osprod( matrix* ptv1, matrix* ptv2 )
{
        register double prod = 0.0, *a, *b, *c;
...
/** \fn double sprod2( matrix* ptv1, matrix* ptv2, int beg1, int end1, int beg2, int end2 )
 * This function calculates the scalar-product of two vectors of equal
 * length. The vectors may be either row- or column- vectors or any 
 * combination of those. (The usual mmul routine can be used to calculate
 ...
 * \return    Scalar product of the two vectors.
 */
double sprod2( const matrix* ptv1, const matrix* ptv2, int beg1, int end1, int beg2, int end2 )
{
        register double prod = 0.0;
        register int i, elements;
...
/** \fn double osprod2( matrix* ptv1, matrix* ptv2, int beg1, int end1, int beg2, int end2 )
 * This function calculates the scalar-product of two vectors of equal
 * length. The vectors may be either row- or column- vectors or any 
 * combination of those. (The usual mmul routine can be used to calculate
 * scalar-products, but it requires a row- and a column-vector.) In this
 ...
 * \todo Test the function for functionality and speed.
 */
double osprod2( matrix* ptv1, matrix* ptv2, int beg1, int end1, int beg2, int end2 )
{
        register double prod = 0.0, *a, *b, *c;
...
/** \fn void mput( matrix *ptm, int row_pos, int col_pos, double value )
...
/** \fn void vput( matrix *ptv, int pos, double value )
 * This function assigns a specified value to a specified element in
 * a vector.
...
/** \fn double mget( matrix *ptm, int row_pos, int col_pos )
 ...
 */
double mget( const matrix *ptm, int row_pos, int col_pos )
{
...
/** \fn double vget( matrix* ptv, int pos )
 ...
 */
double vget( const matrix* ptv, int pos )
{
        register double *pte = NULL;
...
/** \fn void shift( matrix *ptm, double new_element )
...
/** \fn void oshift( matrix *ptm, double new_element )
 * Shifts all elements of a vector and inserts a new value 
 * in the first element. The last element is lost. The
 * function works on both column vectors and row vectors. \n
 ...
 * \todo Test the function for functionality and speed.
 */
void oshift( matrix *ptm, double new_element )
{
        register double *a,*b;
...
/** \fn matrix *mfind( matrix *ptm, double elem )
 ...
 * 
 * \param[in] *ptm Pointer to matrix.
 * \param[in] elem Element to be searched for (double)
...
/** \fn matrix *mnofind( matrix *ptm, double elem )
...
/** \fn void array2mat( matrix *ptm, double *values, int n )
 ...
 * \attention This operation should be used with care.
 */
void array2mat( matrix *ptm, double *values, int n )
{
        if ((ptm->row)*(ptm->col) != n)
                merror("Dimension mismatch in function array2mat");
        memcpy(ptm->mat[0],values,sizeof(double)*n);
}
