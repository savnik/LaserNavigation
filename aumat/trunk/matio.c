...
typedef struct {               /* Matrix structure for MATLAB files */
	...
	long namlen; ///< Leangt of the variable name
} Fmatrix;
...
/** \fn double sload( char file_name[], char scal_name[] )
...
/** \fn double slload( varlisttype *varlist, char scal_name[] )
...
/** \fn int loadmat(FILE *fp,int *type,char *pname,int *mrows,int *ncols,int *imagf,double **preal,double **pimag)
...
/** \fn void savemat(FILE *fp,int type,char *pname,int mrows,int ncols,int imagf,double *preal,double *pimag)
...
/** \fn void matconv( int mn, double *ptd ) 
