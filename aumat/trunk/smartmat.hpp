 ...
 *  TODO: make sure that expressions are not automatically converted to doubles
 ...
 *  scalarexp*vec
 *  vecTrans*scalarexp
 *  TODO: make sure scalarmat behaves like a double when necessary
 */
class tempscalar;
...
protected:
	double data[1][1];
public:
	inline operator const double() const {return data[0][0];}
	inline operator double &() {return data[0][0];}
	inline smartmatBase<1,1> invgauss();
	inline const smartmat<1,1> chol() const;
	inline const double trace() const;
...
public:
	...
	smartmatBase(){}
	explicit smartmatBase(double in){setValues(in);};
	explicit smartmatBase(double * in){setValues(in);};
	smartmatBase(const smartmatBase & in);
	inline int rows(){return row;}
	inline int cols(){return col;}
	inline matrixFiller<row*col,1,(0<row*col)> operator=(double in);
	...
	inline void setValues(int in){setValues((double)in);};
	template<int height, int width>
	...
	smartmat<row,col> ewapply(double (*func)(double));
...
class matrixproxy {
public:
	matrixproxy(double * data, int col) {init(data,col);}
	inline operator const matrix *() const {return &mat;}
	inline operator matrix *() {return &mat;}
protected:
	void init(double * data, int col);
	double * rowpointers[row];
	matrix mat;
};
...
public:
	...
	inline matrixFiller<row*col,1,(0<row*col)> operator=(double in){return smartmatBase<row,col>::operator=(in);}
	...
	explicit smartmat(double in):proxymatBase<row,col>(){setValues(in);}
...
public:
	...
	using smartmatBase<row,col>::setValues;
	explicit smartmat(double in){setValues(in);}
...
public:
	...
	inline smartmat<1,1,false> (double in) {data[0][0] = in;}
	inline smartmat(smartmatBase<1,1> const in) {data[0][0]=in(0,0);};
	inline smartmat<1,1> &operator =(double in)    {data[0][0] = in; return *this;}
	inline smartmat<1,1> &operator += (double rhs) {data[0][0]+=rhs; return *this;}
	inline smartmat<1,1> &operator -= (double rhs) {data[0][0]-=rhs; return *this;}
...
public:
	double val;
	inline tempscalar(double in) {val=in;}
};
...
//////////////////////////// SMARTMATBASE
template<int row, int col>
void smartmatBase<row,col>::setValues(double * in) {
	for(int i=0; i<row; i++)
		for(int j=0; j<col; j++)
		data[i][j] = in[i*row+j];
}
...
template<int row>
const double elementBase<row,row>::trace()const {
	double result=0;
	for(int i=0; i<row; i++)
		result += data[i][i];
	return result;
...
inline const double elementBase<1,1>::trace() const {return data[0][0];}

template<int row, int col>
inline smartmat<row,col> smartmatBase<row,col>::ewapply(double (*fun)(double)) {
...
template<int row, int col>
void smartmatBase<row,col>::setData(double * in) {
...
template<int row, int col>
inline matrixFiller<row*col,1,(0<row*col)> smartmatBase<row,col>::operator=(double in){
...
template<int row>
void matrixproxy<row>::init(double *data, int col) {
    for (int i=0;i<row;i++)
...
private:
	double * dataIn;
public:
	inline matrixFiller(double * data) {
		dataIn = data;
	}
	inline matrixFiller<total,element+1, (element<total)> operator,(double in) {
		dataIn[element] = in;
