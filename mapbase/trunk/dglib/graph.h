...
/*
        If this define DGL_STATS is not defined in this lobrary,
        and is defined when used to make code (in graph.c),
        then the struct is actually smaller then expected, and causes
        memory corruption when using sizeof() to fill the memory with zeros.
        So the safe workaround is to include these values in the struct, used or not.
        chr 22 may 2009
*/
/* #ifdef DGL_STATS */
