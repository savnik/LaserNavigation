 ...
 * Modified by Anders Billesø Beck, DTU 2009 <anders.beck@get2net.dk>
 *
...
ExpatXMLParser::ExpatXMLParser(void) {
  ...
  xml_buffer = (XML_Char *) malloc(sizeof(XML_Char) * xml_buffer_size);
...
// 	free(xml_buffer); //delete xml_buffer;
// 	xml_buffer = NULL;
// 	return;
//   }

  memset(xml_buffer, 0, XML_CHUNK_SIZE);
...
// //   }
// 
//   memset(xml_buffer, 0, chunk_size);
// 
//   /* Set the "ready" flag on this parser */
//   valid_parser = true;
//   XML_SetUserData(expat_parser, (void*)this);
...
/** Reset Expat XML parser
 *
 * This function basically deletes the expat parser and re-creates it
 * to prepare it to read a new file or perform an extra pass on the
 * same file.
 **/
 bool
ExpatXMLParser::resetParser()  { //size_t chunk_size) {
  //Set parser invalid
  valid_parser = false;
...
//   xml_buffer = new XML_Char[xml_buffer_size];
  ...
  /* Allocate a new parser state-object */
  expat_parser = XML_ParserCreate("UTF-8");
...
/*
 * This is an extension for the default handler function above.
 * It accepts a parser as parameter for an external parser-struct
 */
void ExpatXMLParser::register_default_handlers_external(XML_Parser parser) {
  XML_SetElementHandler(parser, &ExpatXMLParser::_element_start_handler,
	  &ExpatXMLParser::_element_end_handler);
  XML_SetCharacterDataHandler(parser,
	  &ExpatXMLParser::_character_data_handler);
  XML_SetProcessingInstructionHandler(parser,
	  &ExpatXMLParser::_processing_instr_handler);
  XML_SetCommentHandler(parser, &ExpatXMLParser::_comment_handler);
  XML_SetCdataSectionHandler(parser,
	  &ExpatXMLParser::_cdata_start_handler,
	  &ExpatXMLParser::_cdata_end_handler);
  XML_SetDefaultHandler(parser, &ExpatXMLParser::_default_handler);
}
	...
	if(bytes_read > 0) {
	  ...
	  if(local_status != XML_STATUS_OK) {
		...
		last_error = XML_GetErrorCode(expat_parser);
                printf("XML Parse error at line %d: %s\n",
                    (int)XML_GetCurrentLineNumber(expat_parser),
