    ...
    switch ( kommando )
    {
      ...
      case       DA :{
         ...
         *gCmdSema = 0;
         u1x = *gXpos;
         u1y = *gYpos;
         if (u1x >xmax ) u1x=xmax;
         if(u1x < xmin ) u1x=xmin;
         if (u1y >ymax ) u1y=ymax;
         if(u1y < ymin ) u1y=ymin;
         dac0SetValue(0x220,u1x+2048);
         dac1SetValue(0x220,u1y+2048);
      ...
      case  LOADKALMAN : *gReturnValue = LoadKalman( DEFAULT,gSprog );
                      *gCmdSema = 0;
                      break;                
    }
}
...
/* Dato       : 11/7 1990                                           */
/********************************************************************/
int LoadKalman( int cmd, int *sprog )
{
  FILE *fp;
  int  i,more;
  char filnavn[16];
  ...
  {
    if(*sprog == DANSK)
       printf("Hent kalmanparametre fra fil : ");
    else if(*sprog == ENGLISH)
       printf("Get kalman parametres from file : ");
    getline(filnavn,16);
    fp=fopen(filnavn,"r");
  }
  if (fp==NULL)
